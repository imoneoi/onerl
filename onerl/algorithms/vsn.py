...
class VSNAlgorithm(Algorithm):
    ...
    def forward(self, obs: torch.Tensor, ticks: int):
        with torch.no_grad():
            ...
            # u = self._predict_w(obs)[:, :, -1]   # ablation 1: use last
    ...
    def learn(self, batch: BatchCuda):
        ...
        with torch.no_grad():
            ...
            update_target = torch.cat([batch.data["rew"][:, -2].unsqueeze(-1),
                                       self.gamma * (1. - batch.data["done"][:, -2]).unsqueeze(-1) * next_w[:, :-1]], -1)
        ...
        loss = torch.mean((w - update_target) ** 2)
